...
const RULES = [
  ...
  // --- availability moved ABOVE experience_start so "earliest/available start
  //     date" (a FUTURE date about this job) matches here, not as a past job's
  //     start date. Also requires availability-specific wording. (Fix 3) ---
  {
    key: 'availability',
    when: (f) =>
      test_norm(
        /availability|disponibilite|earliest.*(start|available)|available.*start|when can you start|start.*(immediately|date).*(available|earliest)/,
        f.label,
        f.name
      ),
  },
  ...
  // experience_title: tightened so a bare "position" anywhere in a sentence
  // (e.g. "how did you hear about this position?") no longer matches. Now
  // requires explicit job-title phrasing. (Fix for the "hear about us" bug)
  {
    key: 'experience_title',
    when: (f) =>
      test_norm(
        /job title|current title|your title|poste actuel|intitule du poste|^role$|^position$/,
        f.label,
        f.name
      ),
  },
  // experience_start: now requires work/employment/job context so it can't
  // swallow a generic "start date" that means availability. (Fix 3) ---
  {
    key: 'experience_start',
    when: (f) =>
      test_norm(
        /(work|job|employment|emploi|poste).*start|start.*(work|job|employment|emploi|poste)|date de debut.*(emploi|poste)/,
        f.label,
        f.name
      ),
  },
  ...
  },
  {
    ...
    when: (f) =>
      test_norm(
        /work auth|authorized to work|right to work|eligible.*work|legally authorized|autorisation.*travail/,
        f.label,
        f.name
      ),
  },
  ...
  // Long free text (big box) — always AI-answered.
  { key: 'free_text', when: (f) => f.type === 'textarea' },
  // Fix 4: a short open-ended text/number input that matched none of the
  // deterministic rules above is an open-ended question (e.g. "How many hours
  // per week?", "How did you hear about us?"). Route it to the AI too, rather
  // than leaving it blank as 'unknown'. Excludes select/radio/checkbox/file,
  // which are handled by the fill layer, not by free-text generation.
  {
    key: 'free_text',
    when: (f) => f.type === 'text' || f.type === 'number',
  },
];
